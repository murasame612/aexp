...
New proposals must not bypass either side of the Result boundary. Every new or touched Result has an incoming `Experiment Design --next_step--> Result` edge so the shared projection can assign it to a hypothesis-led Research Thread. A Result may emit outcome edges only to a Conclusion (`supports`, `weakens`, `does_not_prove`) and/or an Issue (`reveals_issue`); it must not emit an outgoing semantic edge back to a Hypothesis, experiment design, or another Result. Accepted legacy bypasses remain readable and auditable until reviewed reorganization removes them; neutral `related_to/custom` context does not form the research spine.
...
Agent authoring follows the same thread grammar. It reads the shared projection before routing or cleaning a Map, submits semantic cleanup in bounded reviewable proposals, and never writes visual coordinates. Proposal planning returns the candidate overlay's `projected_research`; review requires `eligible=true`, no touched semantic node left in `unassigned`, and the expected Result present in its intended hypothesis Thread. A new or touched Result without its incoming design spine is a contract blocker. Other genuinely incomplete research may remain advisory. Unrelated revision advances are handled by safe proposal rebase.
