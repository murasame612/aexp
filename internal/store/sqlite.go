...
func (s *SQLite) CreateRun(ctx context.Context, r *Run) error {
	r.CreatedAt = time.Now()
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO runs (id, resource_id, name, status, cwd, command, conda_env, env_json, log_paths_json, artifact_paths_json, metric_paths_json, tmux_session, remote_run_dir, exit_code, created_by, created_at, started_at, finished_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		r.ID, r.ResourceID, r.Name, r.Status, r.Cwd, r.Command, r.CondaEnv, r.EnvJSON, r.LogPathsJSON, r.ArtifactPathsJSON, r.MetricPathsJSON, r.TmuxSession, r.RemoteRunDir, r.ExitCode, r.CreatedBy, r.CreatedAt, r.StartedAt, r.FinishedAt,
	)
	return err
}
...
func (s *SQLite) GetRun(ctx context.Context, id string) (*Run, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, resource_id, name, status, cwd, command, conda_env, env_json, log_paths_json, artifact_paths_json, metric_paths_json, tmux_session, remote_run_dir, exit_code, created_by, created_at, started_at, finished_at FROM runs WHERE id = ?`, id).
		Scan(&r.ID, &r.ResourceID, &r.Name, &r.Status, &r.Cwd, &r.Command, &r.CondaEnv, &r.EnvJSON, &r.LogPathsJSON, &r.ArtifactPathsJSON, &r.MetricPathsJSON, &r.TmuxSession, &r.RemoteRunDir, &r.ExitCode, &r.CreatedBy, &r.CreatedAt, &r.StartedAt, &r.FinishedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	return r, err
...
func (s *SQLite) ListRuns(ctx context.Context, filter RunFilter) ([]Run, error) {
	query := `SELECT id, resource_id, name, status, cwd, command, conda_env, env_json, log_paths_json, artifact_paths_json, metric_paths_json, tmux_session, remote_run_dir, exit_code, created_by, created_at, started_at, finished_at FROM runs WHERE 1=1`
	var args []interface{}
...
func (s *SQLite) UpdateRun(ctx context.Context, r *Run) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE runs SET name=?, status=?, cwd=?, command=?, conda_env=?, env_json=?, log_paths_json=?, artifact_paths_json=?, metric_paths_json=?, tmux_session=?, remote_run_dir=?, exit_code=?, created_by=?, started_at=?, finished_at=? WHERE id=?`,
		r.Name, r.Status, r.Cwd, r.Command, r.CondaEnv, r.EnvJSON, r.LogPathsJSON, r.ArtifactPathsJSON, r.MetricPathsJSON, r.TmuxSession, r.RemoteRunDir, r.ExitCode, r.CreatedBy, r.StartedAt, r.FinishedAt, r.ID,
	)
	return err
}
